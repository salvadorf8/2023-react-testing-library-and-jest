...
function Folder({ repoName, owner, folder }) {
    ...
    const children = expanded && childEntries.map((entry) => <TreeEntry key={entry.name} owner={owner} repoName={repoName} entry={entry} />);

    return (
        <div>
            <div className='hover:font-bold'>{name}</div>
            {expanded && <div className={classNames({ 'border-l': expanded }, 'ml-1.5 border-gray-400')}>{children}</div>}
        </div>
    );
}
